...
**Key points:**
- Public routes: `/`, `/guide`, `/privacy`, `/cookies`, `/terms`, `/sign-in`, `/sign-up`, `/unsubscribe`
- Authenticated routes: `/dashboard`, `/budget`, `/balance`, `/plan`, `/transactions`, `/settings`
- All authenticated routes are in the middleware `protectedPaths` list AND each page re-checks session independently
