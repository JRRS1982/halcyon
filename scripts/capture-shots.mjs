...
const seed = async (userId) => {
  ...
  // The ledger shot needs categorised rows across a couple of accounts.
  ...
  const categoryFor = async (label, type, expense, income) =>
    prisma.category.create({
      data: {
        ...
        incomeCategory: income ?? null,
      },
    });

  const groceries = await categoryFor("Groceries", "EXPENSE", "VARIABLE");
  const dining = await categoryFor("Dining out", "EXPENSE", "DISCRETIONARY");
  const transport = await categoryFor("Transport", "EXPENSE", "VARIABLE");
  const salary = await categoryFor("Salary", "INCOME", null, "SALARY");
  ...
  await prisma.transaction.createMany({
    data: [
      ...
      {
        date: day(5),
        description: "Rail season ticket",
        amount: -142.0,
        accountId: current.id,
