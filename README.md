...
Landing-page screenshots live in `public/marketing/` (`dashboard.png`, `budget.png`, `balance.png`, `transactions.png`). They are generated by hand with:

```bash
node scripts/capture-shots.mjs
```

Prerequisites (local only — never run against production):

1. Local DB seeded with 18-month demo data: `make db-seed`
2. App running on `:3210`: `pnpm dev`
3. A signed-in browser session saved at `.auth/state.json` (see the comment in `scripts/capture-shots.mjs`)

**These screenshots must be re-captured whenever the dashboard, budget, balance, or transactions UI changes.** Until they exist, the landing page renders labelled placeholders via `MarketingShot` — the page ships and works correctly without them.
