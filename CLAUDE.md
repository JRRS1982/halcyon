...
- **CI/CD**: GitHub Actions `.github/workflows/ci.yml` has four jobs:
  ...
  - `migrate-prod` — on push to `main` only, gated on the three above; runs `prisma migrate deploy` against production Supabase (`PROD_DIRECT_URL` secret). Vercel waits for this workflow before deploying, so code never goes live against an un-migrated schema. Rollback is one-click in the Vercel dashboard.
