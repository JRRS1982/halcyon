...
- `balance/` — per-period assets/liabilities/net-worth sheet, now rows on user-owned `Account`s (created/archived/deleted from the sheet's Add drawer and delete panel) rather than free-typed lines; `src/lib/balance/` plus `src/lib/accounts/`. See [`docs/features/accounts.md`](docs/features/accounts.md)
