...
- **CI/CD**: GitHub Actions `.github/workflows/ci.yml` has five jobs:
  ...
  - `migrate-prod` — on push to `master` only, gated on lint/integration/e2e; runs `prisma migrate deploy` against production Supabase (`PROD_DIRECT_URL` secret). Vercel waits for this workflow before deploying, so code never goes live against an un-migrated schema. Rollback is one-click in the Vercel dashboard.
