...
- **CI/CD**: GitHub Actions `.github/workflows/ci.yml` has six jobs:
  ...
  - A second workflow, `.github/workflows/monitor.yml`, is not part of CI: it hits production's bearer-gated `GET /api/health` (a `SELECT 1` through the app's own Prisma client) once a day at 08:00 UTC and on `workflow_dispatch`, and fails — which emails the owner — on anything but 200. It needs `CRON_SECRET` as a repo secret matching Vercel's; a 401 means that secret is missing or stale, not that the site is down.
