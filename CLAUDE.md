...
- `plan/` — long-range forecasting: assets, liabilities, incomes, expenses and events projected to death age, with charts and a timeline. Its rows mirror the user's `Account`s and budget `Category`s (`accountId`/`categoryId` on each plan row) and are refreshed only by an explicit Sync — which replaces values, labels and which rows exist, and keeps every assumption; creating a plan is a Sync against an empty plan. `src/lib/plan/`. See [`docs/features/plan-sync.md`](docs/features/plan-sync.md)
...
Shared UI primitives are in `src/components/ui/` (Button, Card, NavBar, …) and the spreadsheet-style grid in `src/components/sheet/`. The Prisma schema has 17 models — eleven core (User, UserSettings, FinancialPeriod, FinancialItem, BalanceItem, BudgetTemplateItem, BalanceTemplateItem, Category, Account, ImportBatch, Transaction) plus the six the Plan feature owns (Plan, PlanAsset, PlanLiability, PlanIncome, PlanExpense, PlanEvent). `Account` is now the durable registry for everything a user owns or owes — `kind`/`category`/`wrapper`/`canImportTransactions`/`linkedAccountId`, with `BalanceItem`/`FinancialItem` observing it via a nullable `accountId` — rather than a transactions-only record; see [`docs/features/accounts.md`](docs/features/accounts.md). `docs/DataModels/DataModels.md` covers what each is for and how ownership flows (not yet updated for this — see that feature doc's "Known gaps"); the schema itself is the reference for columns.
