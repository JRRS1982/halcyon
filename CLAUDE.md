...
- `make dev-db-migrate name=<verb_table>` — `prisma migrate dev --name <name>`; name is required and should start with a verb
...
- **Data access**: Prisma is the ORM. Schema in `prisma/schema.prisma`, migrations in `prisma/migrations/`. In production Prisma talks to Supabase Postgres via two URLs — `DATABASE_URL` (pooled, port 6543, runtime) and `DIRECT_URL` (direct, port 5432, migrations). See ADR-003.
  - **⚠️ Local vs prod DB trap**: env files load in precedence `.env.local` → `.env.development` → `.env`, and `.env.local` holds the **production Supabase** URLs. So anything run **on the host** — `pnpm dev`, `pnpm prisma migrate`, `pnpm db:seed` — hits **production** by default. Develop against the local Postgres by running in **Docker** (`make dev-up`); the container's `compose.yaml` sets both `DATABASE_URL` and `DIRECT_URL` to the local `db` service (precedence over the mounted `.env*`). **Run migrations only inside the container** (`make dev-db-migrate name=<verb_table>`) — `prisma migrate` on the host targets prod via `DIRECT_URL` from `.env.local`. To point the host `pnpm dev` at local, override both URLs in `.env.local` (not `.env`). See README → "Local development & databases".
