...
- `budget/` — per-period sheet (budgeted vs actual) across **four** row kinds: `INCOME` and `EXPENSE` key on a `Category` and carry a `section` (`CategorySection`: Fixed/Variable/Discretionary for expenses, Salary/Side income/Investments/Pensions/Other for income) — one column on `Category`, `BudgetItem` and `PlanExpense`, pinned to `type` by a check constraint; `TRANSFER` (to an `ASSET` account, with a direction) and `REPAYMENT` (at a `LIABILITY` account, never a direction) key on an `Account`. Repayments render *inside* Expenses and count in that total; transfers get their own section. The old transfers panel is gone. `src/lib/budget/`. See [`docs/features/budget-transfers.md`](docs/features/budget-transfers.md)
- `balance/` — per-period assets/liabilities/net-worth sheet. Every row is a user-owned `Account` (created/archived/deleted from the sheet's Add drawer and delete panel) rather than a free-typed line, and the stored fact on the account is `type: AccountType` (`CURRENT_ACCOUNT`, `SAVINGS`, `STOCKS_ISA`, `SIPP`, `MORTGAGE`, …); asset-or-liability (`kind`) and tax wrapper (`wrapper`) are **derived** from it via `kindOf`/`wrapperOf` in `src/lib/accounts/` — computed on demand, no columns store them. The sheet grouping is `Account.section`, user-editable. Each account also carries an `AccountTerms` satellite (nine nullable projection parameters — expected return, fees, interest rate, a mortgage's revision rate, a final-salary entitlement, …; which ones a given `type` prompts for is declared in `ACCOUNT_TYPES`, not fenced in the database) that Plan Sync feeds off. Clicking a row's name opens a card owning name, type, section and terms together; value and notes stay editable in place on the sheet. `src/lib/balance/` plus `src/lib/accounts/`. See [`docs/features/accounts.md`](docs/features/accounts.md)
- `plan/` — long-range forecasting: assets, liabilities, incomes, expenses and events projected to death age, with charts and a timeline. Its rows mirror the user's `Account`s and budget `Category`s (`accountId`/`categoryId` on each plan row) and are refreshed only by an explicit Sync — which replaces values, labels, which rows exist, and the budgeted flow into an account (`monthlyContribution` / `monthlyRepayment`, both monthly — fed by budget transfers and repayments), and keeps every *other* assumption; creating a plan is a Sync against an empty plan. Income tax on the projection is real UK bands (rest-of-UK and Scotland) from `src/lib/tax/`, not a user-typed blended rate — the user only picks a regime and whether thresholds inflate. `src/lib/plan/`. See [`docs/features/plan-sync.md`](docs/features/plan-sync.md), [`docs/features/income-tax.md`](docs/features/income-tax.md)
- `transactions/` — ledger, CSV import (dedupe + reversible batches), categorize; feature-gated by a Settings toggle; `src/lib/transactions/`. The ledger's whole query lives in the URL: a debounced search in the controls card, plus uncategorized / date range / account / category / amount in a filter drawer, all funnelled through one pure predicate (`ledgerWhere`); every drawer filter reports itself as a removable chip. Amounts match **magnitude**, not sign. The controls card is contextual — with rows selected the search box *becomes* the categorize box — and the table's footer nets the rows on screen (labelled `Total ·` or `This page ·` accordingly), not every matching row. See [`docs/features/ledger-filters.md`](docs/features/ledger-filters.md)
...
- **Testing layout**:
  ...
  - Playwright tests live in `e2e/`, under **two** configs that must not overlap. `playwright.config.ts` (`pnpm test:e2e`) runs the suite against a local dev server on `:3100` + the mock Supabase. `playwright.smoke.config.ts` (`pnpm test:smoke`) runs **only** `e2e/smoke/` against an already-deployed URL, with no `webServer` — it is post-deployment verification, fired by `.github/workflows/smoke.yml` on `deployment_status`, never part of `ci.yml` (which finishes before Vercel promotes). The main config therefore carries `testIgnore: "**/smoke/**"`: without it `testDir: "./e2e"` sweeps the smoke suite into the local run, where the bearer-gated `/api/health` has no `CRON_SECRET` and fails on all three engines. Auth tests use a mock Supabase Auth server (`e2e/_mock/supabase.mjs`) rather than a real Supabase project. `playwright.config.ts` runs the mock + a Next.js dev server on port `3100` with env vars pointing at the mock (so it doesn't accidentally hit production).
  - **Browser coverage — and the tests deliberately skipped.** All three engines (chromium, firefox, webkit) run every spec, locally and in CI, and there are no retries: a test that fails once fails the build. The exception is the specs that carry `test.skip(browserName !== "chromium", …)` and so report as skipped on the other two engines. **Every skip in a run summary is one of these**, counted once per non-chromium engine — nothing else in the suite skips, so the skip count is always the gated-test count × 2 and needs no separate tracking. They are the server-action journeys: `transactions`, `budget-transfers`, `reminder`, `dashboard-summary`, `ledger-filter`, `reverse-import`, `marketing-redirect`, `balance-accounts`, `plan-sync`. Each is ~10s of form-filling that ends in a Prisma write, and running the same server action through three engines tests the same server code three times. **Nothing is unrun** — every skipped test executes on chromium in the same job; the skip line is the same tests counted again for engines that were never meant to run them.
  ...
  - **⚠️ One test database, and both suites wipe it.** `pnpm test:int` **and** `make test-e2e` share the same local `halcyon_test`, and both truncate every table between tests (`resetDb` does `TRUNCATE … CASCADE`). Run them at the same time — in one checkout, or from two worktrees, or a subagent's run overlapping yours — and they delete each other's fixtures mid-test. The symptoms do **not** look like contention: you get `Raw query failed. Code: 40P01. Message: deadlock detected` on the TRUNCATE, plus "not found" and wrong-count failures scattered across suites the change never touched. That pattern — failures in unrelated specs, different ones each run — means contention, not a defect. Re-run alone before believing any of it.
    - **Check before starting anything DB-backed:** `pgrep -laf "[p]laywright test|[j]est.*integration"`, and read what it prints. Two traps make the obvious versions lie. A bare path pattern (`pgrep -f "…/halcyon"`) matches your own shell and any editor sitting in the directory. And **any** pattern run through an agent's shell-wrapping tool matches *itself*, because the wrapper's own cmdline contains the pattern text — so `pgrep -laf "halcyon.*(playwright|jest)"` reports busy with nothing running. The `[p]` bracket form fixes both: it still matches `playwright`, but the pattern string itself does not. `docker compose exec -T db psql -U postgres -d halcyon_test -tAc "select count(*) from pg_stat_activity where datname='halcyon_test';"` is the second opinion — live connections when you are not running anything is someone else's suite.
    - CI is unaffected: each job gets its own Postgres service, so this is a local-only hazard.
- **CI/CD**: GitHub Actions `.github/workflows/ci.yml` has six jobs:
  ...
  - `mixed-schema-check` — PR-only. Fails a PR that changes `prisma/migrations/**` **and** application code (`src/**`, `e2e/**`) together, and reports backward-incompatible statements (`DROP COLUMN`, `RENAME`, `SET NOT NULL`, type changes) as a prompt. Run it locally with `pnpm check:mixed-schema`. Override with the `mixed-schema-ok` label. `integration-tests` and `e2e-tests` are both chained behind it (`needs:` + an explicit result guard; `lint-and-test` stays unchained as the fastest, likeliest-to-fail job), so a violating PR is rejected in ~30s rather than after ~5min and ~14min of tests — which makes this check's presence in the **required** list load-bearing, not tidy: a skipped required check counts as satisfied. It has been required since 23 Sep 2026. See [`docs/features/release-flow.md`](docs/features/release-flow.md)
  - `security-audit` — `pnpm audit --audit-level high`. Deliberately **not** a required check and shouldn't become one — a red audit means a new advisory landed against the lockfile, not that the PR is wrong. The aim is to keep it green when possible: fix it promptly (its own small PR is fine) via a scoped override in `pnpm-workspace.yaml` (see the commented examples there — each pins a vulnerable transitive dep to its patched version with the GHSA id and a why-it's-safe note), then `pnpm install` and commit the lockfile.
  - `migrate-prod` — on push to `master` only, gated on lint/integration/e2e; runs `prisma migrate deploy` against production Supabase (`PROD_DIRECT_URL` secret). Rollback is one-click in the Vercel dashboard.
  - A second workflow, `.github/workflows/health.yml`, is not part of CI: it hits production's bearer-gated `GET /api/health` (a `SELECT 1` through the app's own Prisma client) once a day at 08:00 UTC and on `workflow_dispatch`, and fails — which emails the owner — on anything but 200. It needs `CRON_SECRET` as a repo secret matching Vercel's; a 401 means that secret is missing or stale, not that the site is down.
- **Releases are gated in Vercel, not in this repo.** Vercel deploys on push to `master` via its own GitHub App — it is *not* triggered by the workflow — and promotes to production only once its **Deployment Checks** pass: `lint-and-test`, `integration-tests`, `e2e-tests`, `migrate-prod`. That last one is what stops code going live against an un-migrated schema; before 17 Sep 2026 it was absent from the list, which is how PR #195's code reached production while its migration had failed, breaking `/budget` for five days. `security-audit` is deliberately off the list. **The list lives in the Vercel dashboard and is invisible here — changing it changes the release guarantee.** See [`docs/features/release-flow.md`](docs/features/release-flow.md).
- **Merging PRs**: the repo is **public**, and a repository **ruleset** named `master` is active on the default branch (`gh api repos/JRRS1982/halcyon/rulesets`). It requires five status checks — `lint-and-test`, `e2e-tests`, `integration-tests`, `migrate-prod`, `mixed-schema-check` (added 23 Sep 2026) — and blocks deletion and non-fast-forward pushes. There are no bypass actors, so this applies to the owner too: a PR whose checks haven't passed reports `mergeStateStatus: BLOCKED` and cannot be merged by any means, including the API.
  - GitHub's auto-merge is **disabled** at the repo level — `gh pr merge --auto` fails with "Auto merge is not allowed for this repository". Use `gh pr checks <n> --watch` then a plain `gh pr merge <n> --merge --delete-branch` instead. (`--watch` exits non-zero if *any* check fails, required or not — including `security-audit`.)
