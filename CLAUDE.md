...
- `balance/` — per-period assets/liabilities/net-worth sheet. Every row is a user-owned `Account` (created/archived/deleted from the sheet's Add drawer and delete panel) rather than a free-typed line, and the stored fact on the account is `type: AccountType` (`CURRENT_ACCOUNT`, `SAVINGS`, `STOCKS_ISA`, `SIPP`, `MORTGAGE`, …); asset-or-liability (`kind`) and tax wrapper (`wrapper`) are **derived** from it via `kindOf`/`wrapperOf` in `src/lib/accounts/`, with the `kind`/`wrapper` columns kept only as legacy mirrors. The sheet grouping is `Account.section`, user-editable. `src/lib/balance/` plus `src/lib/accounts/`. See [`docs/features/accounts.md`](docs/features/accounts.md)
- `plan/` — long-range forecasting: assets, liabilities, incomes, expenses and events projected to death age, with charts and a timeline. Its rows mirror the user's `Account`s and budget `Category`s (`accountId`/`categoryId` on each plan row) and are refreshed only by an explicit Sync — which replaces values, labels, which rows exist, and the budgeted flow into an account (`annualContribution` / `monthlyRepayment`, fed by budget transfers and repayments), and keeps every *other* assumption; creating a plan is a Sync against an empty plan. Income tax on the projection is real UK bands (rest-of-UK and Scotland) from `src/lib/tax/`, not a user-typed blended rate — the user only picks a regime and whether thresholds inflate. `src/lib/plan/`. See [`docs/features/plan-sync.md`](docs/features/plan-sync.md), [`docs/features/income-tax.md`](docs/features/income-tax.md)
...
Shared UI primitives are in `src/components/ui/` (Button, Card, NavBar, …) and the spreadsheet-style grid in `src/components/sheet/`. The Prisma schema has 17 models — eleven core (User, UserSettings, FinancialPeriod, BudgetItem, BalanceItem, BudgetTemplateItem, BalanceTemplateItem, Category, Account, ImportBatch, Transaction) plus the six the Plan feature owns (Plan, PlanAsset, PlanLiability, PlanIncome, PlanExpense, PlanEvent). `Account` is now the durable registry for everything a user owns or owes — `kind`/`category`/`wrapper`/`canImportTransactions`/`linkedAccountId`, with `BalanceItem`/`BudgetItem` observing it via a nullable `accountId` — rather than a transactions-only record; see [`docs/features/accounts.md`](docs/features/accounts.md). `docs/DataModels/DataModels.md` covers what each is for and how ownership flows (not yet updated for this — see that feature doc's "Known gaps"); the schema itself is the reference for columns.
