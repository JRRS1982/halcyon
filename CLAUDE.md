...
- `plan/` — long-range forecasting: assets, liabilities, incomes, expenses and events projected to death age, with charts and a timeline. Its rows mirror the user's `Account`s and budget `Category`s (`accountId`/`categoryId` on each plan row) and are refreshed only by an explicit Sync — which replaces values, labels, which rows exist, and the budgeted flow into an account (`annualContribution` / `monthlyRepayment`, fed by budget transfers and repayments), and keeps every *other* assumption; creating a plan is a Sync against an empty plan. `src/lib/plan/`. See [`docs/features/plan-sync.md`](docs/features/plan-sync.md)
